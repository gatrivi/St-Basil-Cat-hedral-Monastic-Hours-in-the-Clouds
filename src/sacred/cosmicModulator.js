...
export function getCosmicPhases(dateInput = Date.now()) {
...
export class CosmicResonator {
  constructor() {
    ...
    this.baseFreq = 164.81;
    this.warmth = 0;
    this.sessionProgress = 0;
    this.totalEnrichment = 0;
    this.lfoSpeed = 0.2;
    this._raf = null;
  }

  start({ baseFreq = 164.81, soundEnabled = true } = {}) {
  ...
  _tick() {
    ...
    filter.Q.setTargetAtTime(1 + this.sessionProgress * 2 + cosmic.neptune * 1.5, t, 0.5);

    // Uranus (84y) modulates shimmer depth
    celestialGain.gain.setTargetAtTime(this.sessionProgress * 0.015 + (cosmic.uranus * 0.005), t, 1.0);

    // LFO Shimmer grows as prayer deepens
    lfoGain.gain.setTargetAtTime(this.sessionProgress * 50 + (cosmic.mercury * 20), t, 1.0);
