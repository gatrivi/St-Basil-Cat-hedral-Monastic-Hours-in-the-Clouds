...
interface RoseDrawingProps {
  progress?: number;
  warmthProfile?: number[];
  wiggleProfile?: number[];
  enrichment?: number;
  liveWarmth?: number | null;
  seed?: number;
  size?: number;
  compact?: boolean;
  style?: CSSProperties;
...
export function SacredDrawing({
  ...
  return (
    ...
    >
      ...
      <g transform={`scale(${bloomScale})`} style={{ transformOrigin: '50px 70px' }}>
      {paths.map((d, i) => {
        const start = i / N;
        const end = (i + 1) / N;
        const seg = Math.max(0, Math.min(1, (progress - start) / (end - start)));
